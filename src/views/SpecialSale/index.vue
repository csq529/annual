  ...
  <div class="specialsale-wrapper">
    ...
    <div class="showmore-brand" v-if="showMore">
      ...
      <ul class="more-brand-list">
        <li class="more-item" v-for="(item, index) in moreBrandList" :key="index" @click="chooseBrand(item.id, item.modulename)">
          <img class="brand-img" :src="item.extend1" alt="">
          <span class="brand-name">{{ item.modulename }}</span>
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    getBrand () {
      http.fetchGet(this.$api.FUN).then(res => {
        ...
        data.forEach(item => {
          this.brandList.push(item.modulename)
        })
    ...
    //   this.clearInterval()
    //   this.curTimer = setInterval(this.getDetail, 1000, id)
    // },
    // clearInterval () {
  ...
  created () {
    ...
    this.getBrand()
    this.getDetail(id)
    // this.curTimer = setInterval(this.updateCurTime, 1000, id)
    // this.curTimer = setInterval(this.getDetail, 1000, id)
  },
  mounted () {
